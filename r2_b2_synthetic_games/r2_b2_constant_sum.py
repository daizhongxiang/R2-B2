'''
This script runs the R2-B2 algorithm for two-agent constant-sum games using synthetic payoff functions
'''
...
log_directory = "results_constant_sum_" + level_zero_policy
...
for i in func_list:
    f = all_funcs_info["all_funcs"][i][0]

    def synth_func_1(param): # payoff function of Agent 1
        ind = np.argmin(np.sum(np.abs(domain - param), axis=1))
        # return both the noise observed function value, as well as the noiseless function value
        return np.random.normal(f[ind], obs_noise), f[ind]

    def synth_func_2(param): # payoff function of Agent 2
        ind = np.argmin(np.sum(np.abs(domain - param), axis=1))
        return np.random.normal(1 - f[ind], obs_noise), 1 - f[ind]
